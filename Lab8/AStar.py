...
def cost2(player, enemies, bag): # Cost Function = max(E1 HP, E2 HP) + 10^18 * WillDie - 1000 * Potion
    dmg = 0
    enemy1 = enemies.enemy1
    enemy2 = enemies.enemy2
    WillDie = 0
        ...
        for acts in ["Attack", "Cure", "Fire", "Potion", "Defend"]: # Go through every possible next action
            ...
            if acts == "Attack" or acts == "Fire":
                for target in ["wererat", "spritzer"]: # Selecting target
                    ...
                    if result == "Victory!": # If won, we found the answer, no more astar required
                        answer = temp2
                        break
                    elif result == "You Died. Restart?" or result == "Invalid command":
                        continue # Eliminate impossible sequences
                    if CurCost > cost(player, enemies, bag) + cost2(player, enemies, bag): # If the cost is lower, then we will consider this as our best choice
                        CurCost = cost(player, enemies, bag) + cost2(player, enemies, bag)
                        next = temp2
